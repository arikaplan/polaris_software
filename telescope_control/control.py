...
class interface:
	...
	def moniter_boards(self):
		...
		while self.monitering_boards:
			...
			try:
				self.phidgetTemp = ch.getTemperature()

			except:
				self.phidgetTemp = float('nan')

			#get status bits for calibrator in/out and emulator/encoder
			self.calin = ul.d_bit_in(board_num, port.type, 16)
			self.calout = ul.d_bit_in(board_num, port.type, 17)
			self.acqtype = ul.d_bit_in(board_num, port.type, 18)
	...
	def moniter(self, acq_tel = False):
		...
		while self.monitering:
			...
			self.az, self.el, self.gpstime = gp.getAzEl() [:3]

			#get gps haslock to make sure it is actually obtaining gpstime
			self.gpslock = gp.getAzEl()[3]
			'''
			...
			if acq_tel == False:
				...
				#ra, dec = 0.0, 0.0
				ra, dec = planets.azel_to_radec(self.az, self.el, global_location)
			else:
				ra = 'Off for acq_tel'
				dec = 'Off for acq_tel'
			...
			phTemp = self.phidgetTemp

			#save pointing + science data to Data object
			Data.add(self.az,self.el,self.gpstime, vh1high, vh2high, vh3high, tcryo12, tcryo13, tcryo14, tcryo15, xlevel, ylevel, AZoffset, ELoffset, comptime, flag, relaz, relalt, phTemp)
			...
			if (delta2>=2):
				...
				self.aztxt.insert('1.0', self.az)
				self.alttxt.delete('1.0', END)
				self.alttxt.insert('1.0', self.el)

				self.gpscount.delete('1.0', END)
				self.gpscount.insert('1.0', self.gpstime)
